...
auto CRC::GetCRC8Checksum(const unsigned char *pchMessage, size_t length,
...
*/
auto CRC::VerifyCRC8Checksum(const unsigned char *pchMessage, size_t length) noexcept -> bool {
...
** Output: True or False(CRC Verify Result)
*/
auto CRC::AppendCRC8Checksum(unsigned char *pchMessage, size_t length) noexcept -> void {
    if (pchMessage == nullptr || length <= 2) {
        return;
    }
...
** Output: CRC checksum
*/
auto CRC::GetCRC16Checksum(const uint8_t *pchMessage, size_t length, uint16_t wCRC) noexcept -> unsigned short {
    ...
    while (length--) {
        ...
        wCRC = (static_cast<uint16_t>(wCRC) >> 8) ^
               m_WideCRCTable[(static_cast<uint16_t>(wCRC) ^ static_cast<uint16_t>(chData)) & 0x00ff];
    }
    return wCRC;
}
...
** Output:True or False (CRC Verify Result)
*/
auto CRC::VerifyCRC16Checksum(const uint8_t *pchMessage, size_t length) noexcept -> bool {
    if (pchMessage == nullptr || length <= 2) {
        return false;
    }
...
** Output: True or False (CRC Verify Result)
*/
auto CRC::AppendCRC16Checksum(uint8_t *pchMessage, size_t length) noexcept -> void {
    if (pchMessage == nullptr || length <= 2) {
        return;
    }
